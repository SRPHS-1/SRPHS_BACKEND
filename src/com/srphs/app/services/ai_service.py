...
class AIService:
    ...
    def analyze_health(self, input_dict: dict):
        # Extraer goal antes de pasar al modelo y SHAP, ya que no es parte de la predicción pero sí afecta las recomendaciones
        goal = input_dict.pop("goal", "mantener")
        ...
        result_label = self.encoders["NObeyesdad"].inverse_transform(
            [prediction_numeric]
        )[0]
        ...
        if isinstance(shap_values, list):
            ...
            current_shap = (
                shap_values[0, :, prediction_numeric]
        ...
        # 5. Generar recomendaciones inteligentes con Gemini con meta de personalización basada en el objetivo del usuario
        smart_recs_raw = self.gemini.generate_personalized_recommendations(
            result_label, top_factors, goal=goal
        )
